...
class PostgresConnection
  ...
  def get_track(id, with_positions = true)
    ...
    if with_positions
      ...
      racing_track[:racingTrack][:positions] = positions.map do |pos|
        {
            :position => {
      ...
      end.sort_by { |pos| pos[:position][:timestamp] }
    end
  ...
  def store_position(track_id, timestamp, latitude, longitude)
    ...
    return [false, error(400, 'Timestamp already exists')] if is_existing
